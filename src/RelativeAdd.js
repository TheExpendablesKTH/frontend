...
function RelativeAdd() {
  ...
  const saveRelative = async (e) => {
    ...
    if (phoneRegex.test(phoneToSave.current.value)) {
      await axios.post(apiUrl, { name: nameToSave.current.value, phone: phoneToSave.current.value }, { headers: { 'Content-Type': 'application/json', Authorization: localStorage.getItem('admin_token') } });
    } else {
